...
class IDEC(object):
    ...
    def fit(self, x, y=None, batch_size=256, maxiter=2e3, tol=1e-3, update_interval=140,
...
def load_dataset(path):
    datasets = pd.read_csv(path,header=None)
    datasets = datasets.fillna(999)
    print(datasets)
    ...
    # return tf.convert_to_tensor(datasets)
    # for each_col in datasets.columns[1:]:
    #     datasets[each_col] = datasets[each_col].apply(str_to_list)
    datasets = datasets.iloc[1:,1:]
    print(datasets)
...
if __name__ == "__main__":
    ...
    if args.dataset == 'mnist':  # recommends: n_clusters=10, update_interval=140
        ...
        x = load_dataset(args.custom_path)
